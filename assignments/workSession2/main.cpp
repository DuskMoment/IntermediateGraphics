...
struct
{
	...
	float warp_scale = 1;
	float spec_scale = 1;
	float upper_cutoff = 1.0f;
	float lower_cutoff = 1.0f;
...
int main() {
	...
	ew::Shader planeShader = ew::Shader("assets/doubledash/water.vert", "assets/doubledash/water.frag");
	...
	GLuint wave_spec = ew::loadTexture("assets/doubledash/wave_spec.png");
	GLuint wave_tex= ew::loadTexture("assets/doubledash/wave_tex.png");
	GLuint wave_warp = ew::loadTexture("assets/doubledash/wave_warp.png");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glActiveTexture(GL_TEXTURE1);
		glBindTexture(GL_TEXTURE_2D, wave_warp);

		glActiveTexture(GL_TEXTURE2);
		glBindTexture(GL_TEXTURE_2D, wave_spec);
		...
		planeShader.setInt("warpTex", 1);
		planeShader.setInt("specTex", 2);
