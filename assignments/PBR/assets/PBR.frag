...
uniform vec3 _LightColor = vec3(1.0, 1.0, 0.0);
uniform vec3 _AmbientColor = vec3(0.3,0.4,0.46);

uniform float _Roughness = 0.5;
//camera uniforms
uniform vec3 _EyePos;
uniform float _metalic = 0;
...
vec3 Fresnel(float nDotl, vec3 lambert)
{
	
	vec3 F0 = lambert;
	return F0 + (1-F0) * pow(clamp(1.0 - nDotl, 0.0, 1.0),5); 
}

vec3 CalcPBR(vec3 view, vec3 l)
{
	vec3 Intensity  = _LightColor * 5;
	float nDoth = max(dot(normal,h), 0.0);
	float vDoth = max(dot(view,h), 0.0);
	float nDotl = max(dot(normal,l), 0.0);
	float nDotv = max(dot(normal,view), 0.0);

	vec3 lambert = texture(_MainTex,fs_in.TexCoord).rgb;

	vec3 F = Fresnel(nDotl, lambert/PI);
	vec3 Ks = F;
	vec3 Kd = (1.0 - Ks) * ( 1.0 - _metalic);
